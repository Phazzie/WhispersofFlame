...
- **Dependency Injection**: Adopted `InjectionToken` pattern for all services to ensure strict decoupling between Consumers and Implementations, enabling true "Mock Everything" c- **Test Configuration**: Learned that Angular's `test-setup.ts` can easily duplicate imports if not carefully managed. Always verify the initialization block is unique.
